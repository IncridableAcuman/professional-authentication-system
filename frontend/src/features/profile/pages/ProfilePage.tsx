...
export const ProfilePage: React.FC = () => {
  ...
  const handleAddSocial = async (e?: React.FormEvent) => {
    ...
    if (!user || !trimmed) return;
    if (user.socialLinks?.includes(trimmed)) {
      setErrorMsg("Ushbu ijtimoiy tarmoq havolasi allaqachon qo'shilgan.");
      return;
    }

    const updatedSocials = [...(user.socialLinks || []), trimmed];
    try {
      const updatedUser = await profileApi.editUser(user.id, { username: user.username, socialLinks: updatedSocials });
      setUser(updatedUser);
      setNewSocial('');
      setSuccessMsg("Ijtimoiy havola qo'shildi.");
    } catch (err: any) {
  ...
  const getSocialIcon = (url: string) => {
    ...
    if (lowercase.includes('github.com')) return <Github size={16} className="text-gray-900" />;
    if (lowercase.includes('linkedin.com')) return <Linkedin size={16} className="text-blue-600" />;
    if (lowercase.includes('twitter.com') || lowercase.includes('x.com')) return <Twitter size={16} className="text-sky-500" />;
    if (lowercase.includes('instagram.com')) return <Instagram size={16} className="text-pink-600" />;
    if (lowercase.includes('t.me') || lowercase.includes('telegram')) return <Send size={16} className="text-blue-500" />;
